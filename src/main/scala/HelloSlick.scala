...
object HelloSlick extends App {
  ...
  try {
    ...
    val suppliers: slick.lifted.TableQuery[Suppliers] = TableQuery[Suppliers]

    // the query interface for the Coffees table
    val coffees: slick.lifted.TableQuery[Coffees] = TableQuery[Coffees]

    val setupAction: slick.dbio.DBIO[Unit] = slick.dbio.DBIO.seq(
    ...
    val f = setupFuture.flatMap { _ =>
      ...
      val insertAction: dbio.DBIO[Option[Int]] = coffees ++= Seq (
      ...
      val insertAndPrintAction: dbio.DBIO[Unit] = insertAction.map { coffeesInsertResult =>
      ...
      // Perform the delete
      db.run(deleteQuery.result).map { numDeletedRows =>
